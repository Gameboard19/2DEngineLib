...
void Graphics::DrawRect(float x, float y, float baseLength, float sideLength) {
	renderTarget_->DrawRectangle(D2D1::RectF(x - (baseLength / 2), y - (sideLength / 2), x + (baseLength / 2), y + (sideLength / 2)), brush_, 3.0F);
}
void Graphics::DrawRect(D2D1_POINT_2F topLeft, D2D1_POINT_2F bottomRight) {
	renderTarget_->DrawRectangle(D2D1::RectF(topLeft.x, topLeft.y, bottomRight.x, bottomRight.y), brush_, 3.0F);
}
